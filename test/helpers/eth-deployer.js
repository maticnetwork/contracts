...
import EthDeployer from 'eth-deployer'
...
class Deployer {
  ...
  async initialize() {
    ...
    this.ethDeployer = new EthDeployer.Sender(q)
  }

  getStatus(artifactsDir) {
    let status = {}
    const statusFile = `${artifactsDir}/status.json`
  ...
  getAddressForContract(contract) {
    const status = this._getStatus()
    let address
    Object.keys(status).forEach(id => {
      if (status[id].contract === contract) address = status[id].address
    })
    return address
  }

  async freshDeploy(options = {}) {
    this.ethDeployer.deploy(transformArtifact('Registry', contracts.Registry, []))
    ...
    let _contracts = {
      registry: await contracts.Registry.at(this.getAddressForContract('Registry'))
    }
    // let _contracts = {
...
function transformArtifact(contract, artifact, args) {
