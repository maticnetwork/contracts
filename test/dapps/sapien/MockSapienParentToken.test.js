...
contract.only('SapienChildERC20', async function(accounts) {
  ...
  beforeEach(async function() {
    erc20 = await deployer.deploySapienChildErc20(sapienOwner, { mapToken: false })
  })
  ...
  it('transfer - user allowed in parent', async function() {
    ...
    const parentToken = await deployer.deployMockSapienParentToken()
    await erc20.childToken.setParent(parentToken.address, { from: sapienOwner })
  ...
  it('transferWithPurpose', async function() {
    ...
    await utils.deposit(null, childContracts.childChain, erc20.rootERC20, alice, depositAmount)
    const { receipt } = await erc20.childToken.transferWithPurpose(bob, transferAmount, ['0xaa', '0xbb'], { from: alice })
