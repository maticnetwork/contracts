...
contract('WithdrawManager', async function(accounts) {
  ...
  it('should allow user to start exit after burning tokens', async function() {
    ...
    const withdraw = await web3Child.eth.getTransaction(receipt.transactionHash)
    const withdrawBlock = await web3Child.eth.getBlock(receipt.blockHash, true)
    ...
    burnLogs[0].args._tokenId.should.be.bignumber.equal(
