...
async function stake() {
  ...
  result = await validator.buyVoucher(web3.utils.toWei('100'), await validator.exchangeRate())
...
async function delegate(validatorId) {
  ...
  const validator = await ValidatorShare.at(_validator.contractAddress)
  console.log('approved, delegating now...')
  let result = await validator.buyVoucher(web3.utils.toWei('100'), await validator.exchangeRate())
