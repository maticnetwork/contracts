...
contract ValidatorShare is IValidatorShare {
    ...
    ) public IValidatorShare(_validatorId, _tokenAddress, _stakingLogger) {}
     ...
     */
        uint256 stakePower;
        uint256 valStake;
        (valStake, , , , , , , ) = IStakeManager(owner()).validators(
            validatorId
        ); // to avoid Stack too deep :cry
        ...
        validatorRewards = validatorRewards
            .add(_rewards.mul(commissionRate).div(100))
            .add(_valRewards);
    ...
    function buyVoucher(uint256 _amount) public onlyWhenUnlocked {
        ...
        IStakeManager(owner()).updateValidatorState(
            validatorId,
            int256(_amount)
        );
    ...
    function sellVoucher() public {
        ...
        if (_amount > amountStaked[msg.sender]) {
            ...
            //withdrawTransfer
            IStakeManager(owner()).delegationTransfer(
                validatorId,
                _rewards,
                msg.sender
            );
