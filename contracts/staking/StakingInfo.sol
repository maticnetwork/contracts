...
contract StakingInfo {
  ...
  function logStaked(address signer, uint256 validatorId, uint256 activationEpoch, uint256 amount, uint256 total) public onlyStakeManager {
  ...
  function logSignerChange(uint256 validatorId, address oldSigner, address newSigner) public onlyStakeManager {
  ...
  function logJailed(uint256 validatorId, uint256 exitEpoch) public onlyStakeManager {
  ...
  function logTopUpFee(uint256 validatorId, uint256 fee) public onlyStakeManager {
    emit TopUpFee(validatorId, fee);
  }

  function logClaimFee(uint256 validatorId, uint256 fee) public onlyStakeManager {
    emit ClaimFee(validatorId, fee);
  }

  function getStakerDetails(uint256 validatorId) public view returns(uint256 amount, uint256 activationEpoch, uint256 deactivationEpoch, address signer, uint256 _status) {
    IStakeManager stakeManager = IStakeManager(registry.getStakeManagerAddress());
    IStakeManager.Status status;
    (amount, , activationEpoch, deactivationEpoch, , signer, , status) = stakeManager.validators(validatorId);
    _status =  uint256(status);
  }

  function totalValidatorStake(uint256 validatorId) public view returns (uint256 validatorStake) {
    address contractAddress;
    (validatorStake, , , , , , contractAddress, ) = IStakeManager(registry.getStakeManagerAddress()).validators(validatorId);
